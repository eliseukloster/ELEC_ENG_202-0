...
int main(int argc, char *argv[]) {
    ...
    /* Initialize the weights to the given values */
    if (layer_size == 2) {
        double ws_given[2] = {-0.94, 0.32};
        double ms_given[2] = { 0.87, -0.68};
        for (i=0; i < layer_size; i++) {
            ws[i] = ws_given[i];
            ms[i] = ms_given[i];
            bs[i] = 0;
        }
    } else if (layer_size == 10) {
        double ws_given[10] = {-1.27, 1.13, -1.00, -1.06, 0.86, 0.79, -0.08, 0.21, -0.79, 0.09};
        double ms_given[10] = {0.95, -0.13, 1.28, -1.89, 0.40, 0.32, 1.73, 0.91, -0.97, 0.69};
        for (i=0; i < layer_size; i++) {
            ws[i] = ws_given[i];
            ms[i] = ms_given[i];
            bs[i] = 0;
        ...
        printf("Question 1 only accepts layer sizes of 2 or 10.");
        exit(1);
    }
